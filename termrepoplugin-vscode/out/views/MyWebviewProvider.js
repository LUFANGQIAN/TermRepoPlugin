...
class MyWebviewProvider {
    constructor(extensionUri, storage) {
        ...
        this.mode = null;
        this.disposables = [];
        this.disposables.push(this.storage.onDidChange(() => {
            void this.refresh();
    ...
    async handleMessage(message) {
        switch (message.command) {
            ...
            case 'importWords':
                ...
                return;
            case 'openSettings':
                await vscode.commands.executeCommand('workbench.action.openSettings', 'termrepoplugin-vscode.triggerSymbol');
                return;
            case 'saveTerm':
    ...
    async deleteTerm(id) {
        ...
        const confirmed = await vscode.window.showWarningMessage(`确定删除单词 "${target.originalText}" 吗？`, { modal: true }, '删除');
        if (confirmed !== '删除') {
            return;
        }
    ...
    buildStatePayload() {
        ...
        const selectedTerm = this.mode === 'edit' && this.selectedId ? this.storage.getTerm(this.selectedId) ?? null : null;
        ...
        return {
            ...
            page: this.page,
            mode: this.mode,
            triggerCharacter: (0, termCompletionProvider_1.getTriggerCharacter)(),
        };
    }
    async refresh() {
        if (!this.webviewView) {
